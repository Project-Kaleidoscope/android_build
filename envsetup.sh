...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^kscope_") ; then
        KSCOPE_BUILD=$(echo -n $1 | sed -e 's/^kscope_//g')
    else
        KSCOPE_BUILD=
    fi
    export KSCOPE_BUILD
...
function lunch()
{
    ...
    then
        # if we can't find a product, try to grab it off the KaleidoscopeOS GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/kscope/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/kscope/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
function repopick() {
    T=$(gettop)
    $T/vendor/kscope/build/tools/repopick.py $@
}
